...
class DictSection(Section):
    ...
    def render_as_image(self, width: int=None, section_index: int = 0):
        ...
        for i, (k, v) in enumerate(self.data.items()):
            ...
            canvas = text.write_into_region(canvas, f"{k}", f_region, color=(150, 150, 150), bg_color=(255, 255, 255),
                                            font_size=14, h_align=text.ALIGN_LEFT)
            canvas = text.write_into_region(canvas, f"{v}", f_region, color=(0, 0, 0),
                                            font_size=14, h_align=text.ALIGN_RIGHT)
